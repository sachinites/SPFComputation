...
prepare_new_nxt_hop_template(node_t *computing_node,
    ...
    if(nxt_hop_node && nxt_hop_node->node){ 
        if(nh == LSPNH){ /*It is forward Adjacency next hop*/
            strncpy(nh_template->proxy_nbr_name, 
                    nxt_hop_node->node->node_name, NODE_NAME_SIZE);
        }
        else {
            strncpy(nh_template->nh_name, 
                    nxt_hop_node->node->node_name, NODE_NAME_SIZE);
        }
    }
    else
...
build_routing_table(spf_info_t *spf_info,
    ...
    ITERATE_LIST_BEGIN(spf_root->spf_run_result[level], list_node){
        ...
        sprintf(LOG, "Node %s : processing result of %s, at level %s", 
